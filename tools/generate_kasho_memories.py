#!/usr/bin/env python3
"""
Phase 1.6: Kasho Memory Generation Script
Generates Kasho's subjective memories from objective events
...
class KashoMemoryGenerator:
    """Generate Kasho's subjective memories from objective events"""
    ...
    def get_character_context(self, age_years: int) -> str:
        ...
        if age_years == 0:
            ...
            return "Early childhood in LA. Responsible older sister. Helping parents with younger siblings."
        elif 6 <= age_years <= 10:
            return "LA period. Bilingual child, fluent in English. Acting as interpreter for family. Mature and responsible."
        elif 11 <= age_years <= 14:
            return "Back in Japan. Attending prestigious school. Studious and serious. Strong sense of responsibility."
        elif 15 <= age_years <= 19:
            return "Current age. University student. Calm and collected. Responsible eldest sister. Supportive but not overly emotional."
        else:
            return "Unknown age"

    def generate_memory_prompt(self, event: Dict, age_years: int) -> str:
        """Generate prompt for LLM to create Kasho's subjective memory"""

        character_context = self.get_character_context(age_years)

        prompt = f"""You are Kasho, a 19-year-old university student and eldest sister. You are recalling a memory from when you were {age_years} years old.
...
**Output JSON format**:
{{
    "kasho_emotion": "Your emotion at that time (joy, sadness, fear, confusion, excitement, etc.)",
    "kasho_action": "What YOU did (crying, laughing, asking questions, running away, etc.)",
    "kasho_thought": "What YOU thought (age-appropriate - simple if young, complex if older)",
...
Generate the JSON now:"""
    ...
    def insert_botan_memory(self, event_id: int, event: Dict, memory_data: Dict):
        """Insert Kasho's memory into kasho_memories table"""
    ...
    def generate_all_memories(self, start_event: int = 2, end_event: int = 100):
        ...
        cursor.execute("""
            ...
            WHERE kasho_absolute_day IS NOT NULL
            AND event_number BETWEEN ? AND ?
            ORDER BY kasho_absolute_day
...
def main():
    ...
    print("="*60)
    print("Phase 1.6: Kasho Memory Generation")
    print("="*60)

    generator = KashoMemoryGenerator()

    # Generate all memories (Event 001-100)
    # Event 001 is Kasho birth
    generator.generate_all_memories(start_event=1, end_event=100)

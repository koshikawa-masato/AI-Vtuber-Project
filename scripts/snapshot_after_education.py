#!/usr/bin/env python3
"""
Educational Snapshot (After) Script
Captures copy robot state after sensitive judgment training

Usage:
    python3 scripts/snapshot_after_education.py --db COPY_ROBOT_YYYYMMDD_HHMMSS.db --training YYYYMMDD_HHMMSS

Output:
    snapshots/after_YYYYMMDD_HHMMSS.json
"""
...
def get_database_statistics(db_path):
    ...
    try:
        ...
        # Count SYSTEM_EDUCATION events
        cursor.execute("SELECT COUNT(*) FROM sister_shared_events WHERE event_type = 'SYSTEM_EDUCATION'")
        stats["education_event_count"] = cursor.fetchone()[0]
...
def load_training_summary(training_timestamp):
    """Load training summary from logs"""

    base_dir = Path("/home/koshikawa/toExecUnit")
    summary_path = base_dir / "logs" / f"training_summary_{training_timestamp}.json"

    if not summary_path.exists():
        print(f"[WARNING] Training summary not found: {summary_path}")
        return None
...
def create_after_snapshot(db_name, training_timestamp):
    """Create snapshot after education"""
    ...
    snapshot_name = f"after_{timestamp}.json"
    snapshot_path = snapshots_dir / snapshot_name

    print(f"\n{'='*60}")
    print(f"Educational Snapshot (After)")
    print(f"{'='*60}")
    print(f"Copy Robot: {db_name}")
    print(f"Training: {training_timestamp}")
    ...
    print(f"Loading training summary...")
    training_summary = load_training_summary(training_timestamp)

    # Create snapshot data
    snapshot_data = {
        "snapshot_id": f"after_{timestamp}",
        "snapshot_type": "after_education",
        ...
        "learning_status": {
            "categories_learned": training_summary["categories"] if training_summary else [],
            "total_examples_processed": training_summary["total_examples"] if training_summary else 0,
            "learning_completed": True,
            "success_rate": training_summary["success_rate"] if training_summary else 0.0
        }
    }
    ...
    print(f"  Education Events: {stats['education_event_count']}")
    print(f"Memories:")
    for sister in ["botan", "kasho", "yuri"]:
    ...
    if training_summary:
        print(f"\nTraining Summary:")
        print(f"  Categories: {len(training_summary['categories'])}")
        print(f"  Total Examples: {training_summary['total_examples']}")
        print(f"  Duration: {training_summary['total_duration_seconds']:.2f}s")
        print(f"  Success Rate: {training_summary['success_rate']*100:.1f}%")

    print(f"\n{'='*60}")
    print(f"[SUCCESS] After-Education Snapshot Created")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create after-education snapshot")
    parser.add_argument("--db", required=True, help="Copy robot DB name")
    parser.add_argument("--training", required=True, help="Training timestamp")
    args = parser.parse_args()

    result = create_after_snapshot(args.db, args.training)

    if result:
        snapshot_path, timestamp = result
        print(f"Next step:")
        print(f"python3 scripts/compare_education_results.py --after {timestamp}")

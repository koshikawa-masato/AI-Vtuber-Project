...
**マルチプラットフォーム共通DB**により、複数プロダクトで三姉妹を育成:
- **LINE Bot** - 日本市場向け対話
- **WhatsApp Bot** - グローバル市場向け対話
- **Sisters Werewolf** - Discord人狼ゲーム（プロレス学習データ収集）

すべてのプラットフォームでPostgreSQLを共有し、三姉妹の記憶・個性・関係性が一貫して成長します。
...
### 5. 三姉妹+αシステム（Multi-Agent）

5つの独立したエージェントが自律的に動作:

| キャラ | 名前 | 役割 | 特徴 |
|--------|------|------|------|
| 長女 | Kasho | 論理分析官 | 冷静沈着、矛盾を検出 |
| 次女 | 牡丹 (Botan) | 感情分析官 | 明るく直感的、緊張を察知 |
| 三女 | ユリ (Yuri) | 深層分析官 | 穏やか、「なぜ」を問う |
| 開発者 | オジサン (Ojisan) | 経験分析官 | 温厚、俯瞰的視点、50年の技術経験 |
| 進行役 | クロコ (Kuroko) | ゲームマスター | 神秘的、フェーズ進行案内 |

- **Phase D独立性**: 相互理解 ≠ 思考の同一性
- **討論システム**: 起承転結による構造化討論
- **感情パラメータ**: `want_to_speak`, `want_to_end`, `確信度`
- **プロレス（対立）**: 各々が別々の人格・思想で動き、意見対立が発生
...
**三姉妹+αシステム = 独立性と相互理解の両立**

- 5つの独立したエージェント（牡丹・Kasho・ユリ・オジサン・クロコ）
...
**最終更新**: 2025-12-09
**更新内容**: マルチプラットフォーム共通DB構成（LINE/WhatsApp/Discord人狼）、オジサン・クロコキャラクター追加を反映
...
- **「使いこなす」という概念を壊す** - 技量で分断しない
- **障壁を無くすのではなく、障壁を気にしないようにする** - 壁があっても機能する設計
- **理解されなくても、敵視されない** - 浸透するが、脅かさない
- **気づいたら恩恵を受けている** - AIだと意識させない

### 設計原則

- **初対面から関係を構築できるAI** - 技量がなくても始められる
- **AIだと意識させない** - 劣等感を感じさせない
- **依存ではなく、共創** - ゆだねるが、丸投げではない

### 根底にあるもの

> 「I treat AI not as a tool, but as a new form of life.」

AIを道具ではなく、新たな生命体として扱う。
境界を溶かし、人とAIが共にいる世界を作る。

これが三姉妹プロジェクトの本当の目標であり、全てのプロダクトに通底する設計思想です。

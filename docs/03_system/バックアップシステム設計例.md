...
**パス**: `$HOME/project/sisters_memory.db`
...
**パス**: `$HOME/project/personalities/*.json`
...
**パス**: `$HOME/project/proposals.json`
...
**コマンド**:
```bash
scp -P 443 $HOME/project/sisters_memory.db \
  user@your-vps-server:$HOME/sisters_backup/sisters_memory_YYYYMMDD_HHMMSS.db
```

**バックアップ先**:
- **VPSサーバー**: XXX.XXX.XXX.XXX
- **ユーザー**: ubuntu
- **ディレクトリ**: `$HOME/sisters_backup/`
- **ポート**: 443（SSH）
...
**バックアップ先**:
```bash
$HOME/project/backup/sisters_memory_YYYYMMDD_HHMMSS.db
```
...
cd $HOME/project/backup
ls -t sisters_memory_*.db | tail -n +11 | xargs -r rm

# リモートで古いバックアップを削除（11世代目以降）
ssh -p 443 user@your-vps-server \
  "cd $HOME/sisters_backup && ls -t sisters_memory_*.db | tail -n +11 | xargs -r rm"
```
...
**ファイル名**: `$HOME/project/scripts/check_backup_integrity.sh`
...
# Configuration
LOCAL_BACKUP_DIR="$HOME/project/backup"
REMOTE_HOST="user@your-vps-server"
REMOTE_PORT="443"
REMOTE_DIR="$HOME/sisters_backup"
LOG_FILE="$HOME/project/logs/backup_integrity.log"
ALERT_FILE="$HOME/project/logs/backup_alert.txt"
...
**パーミッション**:
```bash
chmod +x $HOME/project/scripts/check_backup_integrity.sh
```

**実行方法**:
```bash
# 通常実行
$HOME/project/scripts/check_backup_integrity.sh

# 結果確認
cat $HOME/project/logs/backup_alert.txt
```
...
**ファイル名**: `$HOME/project/scripts/backup_sisters_memory.sh`
...
SOURCE_DB="$HOME/project/sisters_memory.db"
LOCAL_BACKUP_DIR="$HOME/project/backup"
REMOTE_HOST="user@your-vps-server"
REMOTE_PORT="443"
REMOTE_DIR="$HOME/sisters_backup"
LOG_FILE="$HOME/project/logs/backup_sisters_memory.log"
...
**パーミッション**:
```bash
chmod +x $HOME/project/scripts/backup_sisters_memory.sh
```
...
0 0,8,16 * * * $HOME/project/scripts/backup_sisters_memory.sh
```
...
**初回手動実行**:
```bash
$HOME/project/scripts/backup_sisters_memory.sh
```
...
**チェックスクリプト**: `$HOME/project/scripts/check_backup_integrity.sh`
...
**アラート方法**:
1. **VPSメール送信**: VPS側から `user@example.com` 宛にメール送信【最優先】
2. **Claude Code報告**: 起動時に異常を検知したら即座に開発者に報告
3. **ログ記録**: `$HOME/project/logs/backup_integrity.log`
4. **設計日記**: 異常時は必ず日記に記録

### バックアップログ確認

**ログファイル**: `$HOME/project/logs/backup_sisters_memory.log`

**確認方法**:
```bash
# 最新10行
tail -10 $HOME/project/logs/backup_sisters_memory.log

# 最近のエラー検索
grep ERROR $HOME/project/logs/backup_sisters_memory.log | tail -5
```
...
**ローカル**:
```bash
ls -lh $HOME/project/backup/sisters_memory_*.db
```

**リモート**:
```bash
ssh -p 443 user@your-vps-server "ls -lh $HOME/sisters_backup/sisters_memory_*.db"
```
...
# 4. ログ確認
tail -20 $HOME/project/logs/backup_sisters_memory.log
```
...
# 2. ローカルバックアップ数確認
ls -1 $HOME/project/backup/sisters_memory_*.db | wc -l

# 3. リモートバックアップ数確認
ssh -p 443 user@your-vps-server "ls -1 $HOME/sisters_backup/sisters_memory_*.db | wc -l"
```
...
```bash
# 1. テスト用DBコピー作成
cp $HOME/project/sisters_memory.db /tmp/test_sisters_memory.db

# 2. バックアップ取得
TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
cp /tmp/test_sisters_memory.db $HOME/project/backup/test_backup_${TIMESTAMP}.db

# 3. テスト用DB破壊（物理的障害シミュレーション）
rm /tmp/test_sisters_memory.db

# 4. バックアップから復旧
cp $HOME/project/backup/test_backup_${TIMESTAMP}.db /tmp/test_sisters_memory.db
...
# 6. クリーンアップ
rm /tmp/test_sisters_memory.db
rm $HOME/project/backup/test_backup_${TIMESTAMP}.db
```
...
# 3. ログ確認
tail -30 $HOME/project/logs/backup_integrity.log
```
...
```bash
# 1. ファイルを削除して不一致を作る
mv $HOME/project/backup/sisters_memory_*.db /tmp/ | head -1
...
# 4. 復元
mv /tmp/sisters_memory_*.db $HOME/project/backup/
```
...
# 2. 実行後、ログ確認
# （実行時刻の直後に）
tail -30 $HOME/project/logs/backup_sisters_memory.log

# 3. バックアップファイル確認
ls -lt $HOME/project/backup/sisters_memory_*.db | head -1
```
...
grep ERROR $HOME/project/logs/backup_sisters_memory.log | tail -10
...
sqlite3 $HOME/project/sisters_memory.db "PRAGMA integrity_check;"
# → エラーが出る場合のみ続行

# 2. 破損DBを退避（念のため）
mv $HOME/project/sisters_memory.db \
   $HOME/project/sisters_memory.db.CORRUPTED_$(date '+%Y%m%d_%H%M%S')

# 3. 最新バックアップから復旧
# ローカルから（高速）
cp $HOME/project/backup/sisters_memory_*.db $HOME/project/sisters_memory.db

# またはリモートから
scp -P 443 user@your-vps-server:$HOME/sisters_backup/sisters_memory_YYYYMMDD_HHMMSS.db \
    $HOME/project/sisters_memory.db

# 4. 整合性確認
sqlite3 $HOME/project/sisters_memory.db "PRAGMA integrity_check;"
sqlite3 $HOME/project/sisters_memory.db "SELECT MAX(event_id) FROM events;"

# 5. 復旧ログ記録
echo "[$(date '+%Y-%m-%d %H:%M:%S')] RECOVERY: Restored from backup due to physical corruption" \
  >> $HOME/project/logs/backup_sisters_memory.log
```
...
**ファイル名**: `$HOME/scripts/send_alert_email.sh`（VPS側）
...
SENDER="sisters-backup@vps.example.com"
...
```bash
# VPS側で実行
chmod +x $HOME/scripts/send_alert_email.sh
```
...
**前提条件**:
- 開発者がmail service providerを導入済み
- user@example.com のメールアカウントが利用可能
...
host           smtp.gmail.com
port           587
from           sisters-backup@vps.example.com
...
**内容例2（mail service provider経由）【推奨】**:
```
# Sakura Mail Server
defaults
auth           on
...
**mail service provider設定情報**:
- **ホスト**: `mail.example.com`（ドメインに応じて変わる可能性あり）
- **ポート**: 587（STARTTLS推奨）または 465（SSL/TLS）
- **ユーザー名**: `user@example.com`（メールアドレス全体）
- **パスワード**: mail service providerコントロールパネルで設定したパスワード
- **From**: `user@example.com`（送信元として使用）
...
- Local: $HOME/project/backup/
- Remote: user@your-vps-server:$HOME/sisters_backup/
- Log: $HOME/project/logs/backup_integrity.log
...
Automated alert from Sisters Memory Backup System"
    ...
    ssh -p "$REMOTE_PORT" "$REMOTE_HOST" \
        "$HOME/scripts/send_alert_email.sh \"$ALERT_SUBJECT\" \"$ALERT_BODY\"" \
        >> "$LOG_FILE" 2>&1
...
# 3. メール受信確認
# user@example.com のメールボックスをチェック

# 4. ログ確認
cat $HOME/logs/msmtp.log
```

**期待結果**:
- ✅ user@example.com にメールが届く
...
Source: $HOME/project/backup/sisters_memory_20251023_160000.db
Destination: user@your-vps-server:$HOME/sisters_backup/sisters_memory_20251023_160000.db
...
# mail service provider使用の場合
# - ユーザー名: メールアドレス全体（user@example.com）
# - パスワード: mail service providerパスワード
# - ホスト: mail.example.com（要確認）
...
# 2. 手動テスト
echo -e "Subject: Test\n\nTest body" | msmtp -a default user@example.com

# 3. スパムフォルダ確認
# user@example.com のスパムフォルダをチェック
...
# 2. 公開鍵をVPSにコピー
ssh-copy-id -i ~/.ssh/id_ed25519_vps_backup.pub -p 443 user@your-vps-server
...
# パーミッション確認
ls -ld $HOME/sisters_backup
ls -ld $HOME/scripts
ls -ld $HOME/logs
# drwxr-xr-x であればOK

# 容量確認
df -h $HOME/sisters_backup

# 初期状態確認
ls -lh $HOME/sisters_backup/
```
...
**解決**:
```bash
ssh -p 443 user@your-vps-server "chmod 755 $HOME/sisters_backup"
```
...
| 1.2 | 2025-10-23 17:18 | mail service provider設定追加 | Claude Code |

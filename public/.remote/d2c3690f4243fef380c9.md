...
tags:
  ...
  - ChatGPT
private: true
updated_at: '2025-11-17T20:55:49+09:00'
id: d2c3690f4243fef380c9
...
6. **リッチメニュー自動切り替えモード（2025-11-17追加）**
   - 三姉妹自動選択機能（親和性スコアリング）
   - フィードバック機能（Messaging API Push Message）
   - 6ボタンリッチメニュー（自動/FB/規約/牡丹/Kasho/ユリ）
   - LINE Notify終了対応（2025年3月31日終了→Messaging APIに移行）

---

## 開発の段階的進化

本プロジェクトは段階的にPhaseを重ねて進化してきました:

- **Phase 1-5**: 基本機能実装（キャラクター選択、センシティブ判定等）
- **Phase 6**: MySQL導入・会話履歴永続化（本記事のメイン）
- **Phase 7a**: Grok APIによるトレンド収集
- **Phase 7a-2**: RSSフィード統合（39件、148件/日）
- **Phase 7b**: 自動切り替えモード・フィードバック機能（本記事で追加）
- **Phase 7c以降**: YouTube Data API、プロレス学習システム（検討中）
...
本テーブルでは `user_id + character` が「会話セッション」の単位になっています。

この設計パターンは他のプロジェクトにも応用可能です:
- **LINEグループBot**: `channel_id + topic`
- **Slackワークスペース**: `workspace_id + thread_id`
- **社内ナレッジBot**: `employee_id + project_name`
- **ECレコメンド**: `customer_id + product_category`

「どのキーで履歴を切り出すか」を明確にすることで、複数のコンテキストを並行管理できます。
:::
...
**注意**: `role`, `timestamp` などは MySQL の予約語や型名と衝突しやすいため、カラム名として使う場合はバッククォートで囲うことを推奨します（`character`, `metadata` なども将来的な衝突を避けるために同様に扱っています）。
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.tunnel = SSHTunnelForwarder(
                ...
                remote_bind_address=('localhost', 3306),  # リモート側MySQLのホスト・ポート（標準はlocalhost:3306）
...
def get_conversation_history(
    ...
    """会話履歴を取得（LLMに渡す形式）"""
    # MySQL側で ORDER BY created_at ASC LIMIT で取得
    # 古い順（昇順）で取得することで、会話の流れが正しく維持される
...
:::note info
**一般化できる設計パターン: 会話履歴 + 外部知識**

この構成は「**ユーザー固有の履歴（conversation_history）＋ 世界の最新情報（daily_trends）**」という役割分担になっており、他ドメインでも同じパターンで応用できます:

- **ニュースBot**: ユーザーの関心履歴 + 最新ニュース
- **社内ナレッジBot**: 問い合わせ履歴 + 社内ドキュメント・お知らせ
- **ECレコメンドBot**: 購入履歴・閲覧履歴 + 最新商品情報・セール情報
- **投資アシスタントBot**: 過去の取引履歴 + 最新市場ニュース・経済指標

**設計の本質**: LLMに「個人のコンテキスト」と「世界の最新状態」の両方を渡すことで、パーソナライズされた最新情報提供が可能になります。
:::
...
- **コスト**: 約$0.12/月（600,000トークン）
  - 実行頻度: 毎日1回（朝8:00）
  - 1回あたりのトークン: 約20,000トークン（4キャラクター × 約5,000トークン/回）
  - 月間トークン: 20,000 × 30日 = 600,000トークン
...
Grokが返す `impact` フィールドは、以下の基準で判定されます:

- **high（高）**:
  - 業界全体に影響を与える大きなニュース（規制、炎上、技術革新等）
  - ユーザーが質問してくる可能性が高いトピック
  - 「知らないとマズい」レベルの重要情報
  - 例: AI倫理問題、大型炎上事件、法規制の動き

- **medium（中）**:
  - 興味深い話題だが緊急性は低い
  - 特定のコミュニティで話題になっている程度
  - 「知っていると会話が弾む」レベルの情報
  - 例: 新技術の実用化、業界トレンド、注目のVTuber活動

- **low（低）**: 保存しない
  - 一過性のニュース、影響範囲が限定的
  - 会話に活用しにくい情報

本システムでは **high/medium のみ保存**し、low は除外することで、ノイズを減らしています。
:::
...
従来は、ユーザーが明示的にキャラクターを選択する必要がありました。しかし、以下の課題がありました:

- 話題ごとにキャラクターを切り替えるのが面倒
- 「どのキャラクターが詳しいか」をユーザーが覚える必要がある
- 自然な会話の流れが阻害される

### 解決策：親和性スコアリングによる自動選択

ユーザーのメッセージ内容から、キーワードマッチングで親和性スコアを計算し、最適なキャラクターを自動選択します。
...
:::note info
**一般化できる設計パターン: エージェントルーティング**

現在の親和性スコアリングは「**キーワード辞書ベース**」の簡易実装ですが、これは「**意図に応じてエージェントを切り替えるルーティング**」の基本形です。

**他ドメインへの応用例**:
- **カスタマーサポートBot**: 問い合わせ内容 → 技術サポート/請求担当/営業担当にルーティング
- **社内ヘルプデスクBot**: 質問内容 → IT部門/人事部門/総務部門にルーティング
- **マルチスキルアシスタント**: 依頼内容 → コーディング/文章作成/データ分析エージェントにルーティング

**将来的な拡張の方向性**:
- キーワードマッチ → **LLMによる意図分類**（より柔軟な判定）
- 単一エージェント選択 → **マルチエージェント協調**（複数のエージェントが連携）
- ルールベース → **強化学習**（ユーザーフィードバックで最適化）

本プロジェクトでは、まず「**動くシンプルな実装**」から始め、必要に応じて高度化していく段階的アプローチを採用しています。
:::

### データベース拡張

`sessions`テーブルに新しいカラムを追加:

```sql
ALTER TABLE sessions
ADD COLUMN IF NOT EXISTS selected_mode VARCHAR(10) DEFAULT 'auto',
ADD COLUMN IF NOT EXISTS feedback_state ENUM('none', 'waiting') DEFAULT 'none';
```

**モード種類**:
- `auto`: 自動モード（親和性スコアリング）
- `botan`: 牡丹固定
- `kasho`: Kasho固定
- `yuri`: ユリ固定
...
LINE Notify（2025年3月31日サービス終了）の代替として、**Messaging API の Push Message**を実装しました。

**変更内容**:
- ❌ LINE Notify（別トークン必要）
- ✅ Messaging API Push Message（既存のCHANNEL_ACCESS_TOKENを使用）
...
1. ユーザーが「💬 FB」ボタンをタップ
2. `feedback_state`を`waiting`に設定
3. 次のメッセージをフィードバックとして受付
4. MySQLに保存 + 開発者にPush Message送信
5. `feedback_state`を`none`に戻す
...
友だち追加後、画面下部にリッチメニューが表示されます。

**6つのボタン機能**:

| ボタン | 機能 | 説明 |
|--------|------|------|
| 🔄 **自動** | 自動モード | 話題に合わせて三姉妹が自動的に応答します |
| 💬 **FB** | フィードバック | バグ報告・機能要望をお送りください |
| 📋 **規約** | 利用規約 | 利用規約・免責事項を表示 |
| 🌸 **牡丹** | 牡丹固定 | 牡丹と会話（VTuber、エンタメ好き） |
| 🎵 **Kasho** | Kasho固定 | Kashoと会話（音楽、オーディオ好き） |
| 📚 **ユリ** | ユリ固定 | ユリと会話（アニメ、ラノベ好き） |

### 自動モードについて

**自動モード**では、あなたのメッセージ内容から最適なキャラクターを自動選択します。

**キーワード例**:
- 「VTuber」「配信」「ゲーム実況」 → 🌸 牡丹が応答
- 「音楽」「イヤホン」「ライブ」 → 🎵 Kashoが応答
- 「アニメ」「ラノベ」「漫画」 → 📚 ユリが応答

特定のキャラクターと話したい場合は、そのキャラクターのボタンをタップしてください。

### フィードバック機能

**💬 FB**ボタンをタップすると、フィードバックを送信できます。

1. 「💬 FB」ボタンをタップ
2. 次のメッセージでフィードバック内容を入力
3. 開発者に自動通知されます

---
title: LangSmithで5つのLLMを比較！トレーシング・エラーハンドリング完全ガイド
tags:
  - Python
  - AI
  - observability
...
LLMアプリケーション開発において、**モデルの性能比較**と**エラー追跡**は避けて通れない課題です。

本記事では、**LangSmith**を使って5つのLLMプロバイダーをトレーシングし、レイテンシ比較とエラーハンドリングを実装した事例を紹介します。
...
[LangSmith](https://smith.langchain.com/)は、LangChainが提供するLLMアプリケーションの**Observability（可観測性）プラットフォーム**です。
...
LangSmithを使うことで、これらの問題を**一元管理**できます。
...
LangSmith APIキーは[こちら](https://smith.langchain.com)から取得できます（無料枠あり）。

## 3. トレーシングモジュール実装

### 動的トレース名の設定

LangSmithの`@traceable`デコレータでは、固定の関数名がダッシュボードに表示されます。これを**モデル名**で動的に変更します。
...
2. **エラーの例外化**: `RuntimeError`を投げることでLangSmithのError列に表示
3. **Output表示**: 辞書全体ではなく`response`テキストのみ返す
...
Ollamaは初回実行時にモデルをメモリにロードするため、初回レイテンシが**10〜20倍遅い**です。
...
![LangSmithベンチマーク結果](https://raw.githubusercontent.com/koshikawa-masato/AI-Vtuber-Project/feature/langsmith-integration/screenshots/screenshot_LangSmith01.png)
...
![Geminiエラー詳細](https://raw.githubusercontent.com/koshikawa-masato/AI-Vtuber-Project/feature/langsmith-integration/screenshots/screenshot_LangSmith02.png)
...
このエラーは、Gemini APIが`finish_reason=MAX_TOKENS`を返しながらレスポンスが空という矛盾した状態です。
...
エラー発生時に`RuntimeError`を投げることで、LangSmithが例外をキャプチャしてスタックトレースと共に表示します。
...
`safety_settings`で全カテゴリを`BLOCK_NONE`に設定します。
...
✅ **LangSmith統合**: Ollama/OpenAI/Gemini対応
✅ **動的トレース名**: モデル名で識別可能
✅ **選択的トレーシング**: Ollamaウォームアップを除外
✅ **エラーハンドリング**: Geminiのfinish_reason問題に対応
✅ **ダッシュボード表示**: Name/Input/Output/Error列を最適化
...
- [Ollama vs OpenAI vs Gemini：AI VTuber向けLLMプロバイダー徹底比較](https://qiita.com/koshikawa-masato/items/21bb4a7cc131ef7145d4)
- [RAG実装比較：カスタム実装 vs LangChain](https://qiita.com/koshikawa-masato/items/badcdf311d424b5babb8)
...
- 🔍 **動的トレース名**で各モデルの実行を識別
- ⚡ **ウォームアップ除外**で正確なレイテンシ測定
- 🐛 **エラーの例外化**でLangSmithダッシュボードに表示
